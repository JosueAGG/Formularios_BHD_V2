...
public class Evaluacion_impacto_DPIA<T extends TableEntitySelection> implements UserService<T> {
	...
	private void pane(UserServicePaneContext aPaneContext, UserServicePaneWriter aWriter) {
		...
		//Desplegar datos basicos del tratamiento
		aWriter.startExpandCollapseBlock(UserMessage.createInfo("Datos básicos del tratamiento:"), true);
		//Titulo de datos basicos del tratamiento
		aWriter.add_cr("<h3>Datos básicos del tratamiento:</h3>");
		//Despliega campos este titulo va basio 
		aWriter.startExpandCollapseBlock(UserMessage.createInfo(""), true);
		//Campoos que se mostrara como identificador, resumen ejecutivo, vigente del banco por etc...
